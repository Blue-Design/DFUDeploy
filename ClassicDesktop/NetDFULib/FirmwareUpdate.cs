...
namespace STDFULib
{
	public class FirmwareUpdate : Win32Usb, IFirmwareUpdate
	{
		...
		public bool IsDFUDeviceFound()
		{
            ...
            //
            foreach (MappingSector sector in sectors)
            {
                Console.WriteLine("Index: " + sector.dwSectorIndex + "\n" +
                    "Size: " + sector.dwSectorSize + "\n" +
                    "Address: " + sector.dwStartAddress + "\n" +
                    "Name: " + sector.Name + "\n" +
                    "Type: " + sector.Type);
            }
		...
		private void DoFirmwareUpdate()
		{
			...
			try
			{
	            ...
	            if (ExitDFUMode)
                {
                    ...
                    Detach(hDevice, 0x0800C000);	//this makes me feel too dirty, so I'm turning it off for now  was 0x08020000
                }
		...
		private IntPtr OpenDFU_Device(out List<MappingSector> Sectors, out UInt16 MaxWriteBlockSize)
		{
			...
			try
			{
				...
				while (i-- > 0)
				{
					...
					if (0 == Index)
					{
						System.Threading.Thread.Sleep(50); //Was 500
					}
					else
					{
						break;
